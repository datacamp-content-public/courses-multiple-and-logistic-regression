...
`@script`
Either you love japanese beaches but are just afraid of Wolf spiders, or you love nature and you want to save wolf spiders from goin instinct. Either case, a linear model is needed in order to help you find out if the beach you plan to go to is inhabited by Wolf spiders or what would be the kind of sand that the Wolf spiders thrive and survive extinction.
...
`@script`
After having covnerted all entries into numeric type, we can use seaborn package to fit the best fit line on the left using ordinary-least squares in linear regression and using maximum likelihood on the right in logistic regression.
...
`@script`
As seaborn is only a visualization package, we use statsmodel to retrieve the best-fit model parameters, the slope and intercept, as well as the significance in the relationship between explanatory and output variables.
...
`@script`
and from statsmodel also obtain the predicted values from the best-fit models.
...
`@script`
To put all in one slide, the best-fit equations (top panel), their plots (middle panel), and predicted values (bottom panels)
...
`@script`
we can immediately perform a visual check about the predicted values. As highlighted in red, we obtain non-sense values of probabilities, that is, greater than 1, because of using linear regression to model a categorical output variable, which is in this case, a binary one; the chances of finding or not finding a Wolf spider based on the grain size of beach's sand.
...
`@script`
The rule of thumb whether to pursue the usual linear regression or the special case on logistic regression in handling categorical variables, either binary (or ordinal and nominal outcomes), depends on the values of probabilities. If they are extremely close to 0 or 1, then go for logistic regression. Otherwise, values of probabilities between 0.20 and 0.80 linear and logistic models work equally well, though the linear model is preferred as being easy to interprete.
