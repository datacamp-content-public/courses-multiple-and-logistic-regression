...
`@script`
To avoid errors in handling strings, we need to convert all string type occurrences in our Wolf spider data to numeric types using sklearn label encoder; which does nothing but turning to 0 and 1.
...
`@script`
After having converted all entries into numeric type, we can directly use seaborn package to find the best fit line on the left using ordinary-least squares in linear regression and using maximum likelihood on the right in logistic regression.
