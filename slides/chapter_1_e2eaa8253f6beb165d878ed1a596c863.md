...
`@script`
As this screencast is about on logistic regression modelling in Python language, the following are the four basic Python packages that we will be using:
...
`@script`
For you considering to visit a beach that has an average grain size of 0.3 mm, the linear regression model would predict the presence of Wolfspiders, which in contrast in logistic regression model would predict the absence of Wolfspiders.
