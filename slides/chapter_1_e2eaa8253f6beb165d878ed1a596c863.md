...
`@script`
As this screencast is about using the Python language in logistic regression modelling, the following are the four basic Python packages that we will be using:
* pandas for reading data stored in files
* statsmodels for linear and logistic regression modelling
* numpy for operations on arrays
* matplotlib and seaborn for simple plotting and visualization
...
`@script`
A picture shown on the left shows a wolf spider coming out of its own burrows, and below the picture is a summary text describing the wolf spider's behavior and a declaration of being an endangered species  and of having a high risk of extinction. For our own puposes, we will use the Wolf spider data that a team of Japanese researchers in the year 2006 had collected in order to investigate what would cause wolf spiders to be found on a beach. On the right side shows a code how we would be able to read the file using Python's pandas module; here indicating the name of the file and the column separator, which in this case, is a semi-colon. We can type the list command to identify the column header labels.
...
`@script`
Before being able to perform further visualisation and analysis, we have to make sure that the entries in our data are all of numeric type. As shown in the left, the variable spiders originally has either the string value absent or present. By using pandas' get_dummies() function, we can binarize and obtain only 0 or 1 values, depending if we meant that the spiders are absent (right-hand side, first column to the right) or that the spiders are present (second column to the right).
...
`@script`
We can use Python's statsmodel package, the OLS, which stands for ordinary least squares in the case of linear regression (left panel) and the Logit, for logistic regression (right panel). We have used the same input variables X and y, where in the X variable we have added a constant using statsmodels tool.
