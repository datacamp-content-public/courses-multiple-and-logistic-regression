...
`@script`
Before being able to perform further visualisation and analysis, we have to make sure that the entries in our data are all of numeric type. As shown in the left, the variable spiders has either the string value absent or present. However, by using pandas get_dummies() function, we can binarize and obtain only 0 or 1, depending if we meant spiders are absent (first column to the right) or present (second column to the right)
